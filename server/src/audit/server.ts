import type { IAuditStore, AuditListQuery } from "./types";
...
export function startAuditServer(store: IAuditStore, port: number, opts?: AuditServerOptions): Promise<{ server: any; actualPort: number }> {
  ...
  async function handleFimDemo(req: any, res: any) {
  ...
  function serveDemoConfig(res: any) {
    try {
      const config = readAutocompleteConfig();
...
function serveDashboard(res: any) {
  res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
  res.end(DASHBOARD_HTML);
}

function serveRecordList(store: IAuditStore, url: URL, res: any) {
...
function serveRecordDetail(store: IAuditStore, id: string, res: any) {
  const record = store.get(id);
  if (!record) { sendJson(res, 404, { error: "Not found" }); return; }
  sendJson(res, 200, record);
}

function serveStats(store: IAuditStore, res: any) {
...
function serveSSE(store: IAuditStore, req: any, res: any) {
  res.writeHead(200, {
...
export function stopAuditServer(server: any) {
