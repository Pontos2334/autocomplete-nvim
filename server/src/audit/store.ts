import type { AuditRecord, AuditListQuery, IAuditStore } from "./types";
...
const LIST_COLUMNS = [
  ...
  "num_lines", "preview_only", "timed_out", "chunk_count",
  "filter_reason", "timing",
].join(", ");

const JSON_FIELDS = [
  "timing", "snippet_summary", "completion_options", "error",
  "prefix", "suffix", "prompt", "completion", "processed_completion", "displayed_completion",
];
  ...
  for (const [k, v] of Object.entries(row)) {
    ...
    if (JSON_FIELDS.includes(camelKey) && typeof v === "string") {
      try { r[camelKey] = JSON.parse(v); } catch { r[camelKey] = v; }
                ...
                camelKey === "cacheHit" || camelKey === "previewOnly" || camelKey === "timedOut") && typeof v === "number") {
      r[camelKey] = !!v;
    } else {
      r[camelKey] = v;
    }
...
export function createAuditStore(options: {
  ...
  function updateRecord(id: string, updates: Partial<AuditRecord>) {
    ...
    // Merge JSON fields with existing data
