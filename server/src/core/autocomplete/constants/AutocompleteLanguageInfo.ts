...
export interface AutocompleteLanguageInfo {
  ...
  /**
   * Optional line-wrapping comment style used when we inject context snippets
   * into the prompt for languages that do not have a natural single-line comment.
   */
  contextComment?: {
    prefix: string;
    suffix?: string;
...
export function formatLineComment(
  ...
  if (language.contextComment) {
    return `${language.contextComment.prefix}${text}${language.contextComment.suffix ?? ""}`;
  }

  if (language.singleLineComment !== undefined) {
    return `${language.singleLineComment} ${text}`;
  }
...
export function formatCommentBlock(
  ...
  return text
    .trim()
    .split("\n")
    .map((line) => formatLineComment(language, line))
    .join("\n");
}
...
export const Html: AutocompleteLanguageInfo = {
  name: "HTML",
  topLevelKeywords: ["html", "body", "head", "script", "style", "div"],
  singleLineComment: "<!--",
  contextComment: {
    prefix: "<!-- ",
    suffix: " -->",
