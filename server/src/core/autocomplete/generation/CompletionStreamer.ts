...
import { StreamPreviewState } from "../util/types";
...
export class CompletionStreamer {
  ...
  async *streamCompletionWithFilters(
    ...
    helper: HelperVars,
    previewState: StreamPreviewState,
  ) {
    if (!llm.supportsFim()) {
      throw new Error(
        `Autocomplete requires a FIM-capable model. Chat fallback is disabled. Configure a FIM endpoint such as DeepSeek beta (${llm.apiBase ?? "missing apiBase"}).`,
      );
    }
