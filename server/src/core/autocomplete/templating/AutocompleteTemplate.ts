...
import { describeFile } from "./formatting.js";
import {
...
const deepseekFimTemplate: AutocompleteTemplate = {
  // DeepSeek V4 FIM API natively supports prompt + suffix.
  // Inject snippets as raw code with file-path separators (no comment wrapping).
  compilePrefixSuffix: (prefix, suffix, filepath, reponame, snippets, workspaceUris) => {
    const codeSnippets = snippets.filter(
      (s) => s.type === AutocompleteSnippetType.Code,
    ) as AutocompleteCodeSnippet[];
    if (codeSnippets.length === 0) {
      return [prefix, suffix];
    }
    const snippetBlock = codeSnippets
      .map((s) => {
        const relPath = getLastNUriRelativePathParts(workspaceUris, s.filepath, 2);
        const desc = describeFile(s.filepath);
        return `--- [${desc}: ${relPath}] ---\n${s.content}`;
      })
      .join("\n\n");
    const divider = `--- 以下是当前编辑文件 (${getLastNUriRelativePathParts(workspaceUris, filepath, 2)}) ---`;
    prefix = `${snippetBlock}\n${divider}\n${prefix}`;
    return [prefix, suffix];
  },
  template: "{{{prefix}}}",
