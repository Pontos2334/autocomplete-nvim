...
export const getSnippets = (
  ...
  const snippets = {
    ...
    base: filterSnippetsAlreadyInCaretWindow(
      [
  ...
  const snippetConfigs: {
    ...
    },
    {
      ...
      defaultPriority: 2,
      snippets: payload.recentlyOpenedFileSnippets.map((s) => ({
        ...s,
        content: `[最近打开的文件]\n${s.content}`,
      })),
    },
    {
      ...
      defaultPriority: 3,
      snippets: payload.recentlyVisitedRangesSnippets.map((s) => ({
        ...s,
        content: `[最近访问的代码]\n${s.content}`,
      })),
    ...
    },
    {
      ...
      defaultPriority: 4,
      snippets: payload.recentlyEditedRangeSnippets.map((s) => ({
        ...s,
        content: `[最近编辑的代码]\n${s.content}`,
      })),
    },
    ...
    },
    {
      ...
      snippets: filterSnippetsAlreadyInCaretWindow(
        [
          ...payload.rootPathSnippets.map((s) => ({
            ...s,
            content: `[当前作用域上下文]\n${s.content}`,
          })),
          ...payload.importDefinitionSnippets.map((s) => ({
            ...s,
            content: `[Import 定义]\n${s.content}`,
          })),
          ...payload.staticSnippet.map((s) => ({
            ...s,
            content: `[类型定义上下文]\n${s.content}`,
          })),
        ],
