...
function pruneLength(
  ...
  const requestTokenCount = llm.supportsFim()
    ? countTokens(prefix, llm.model) + countTokens(suffix, llm.model)
    : countTokens(prompt, llm.model);
  return requestTokenCount - maxAllowedPromptTokens;
}
...
export function renderPromptWithTokenLimit({
  ...
  if (llm) {
    let activePrefix = compiledPrefix;
    let activeSuffix = compiledSuffix;
    const prune = pruneLength(llm, prompt, activePrefix, activeSuffix);
    if (prune > 0) {
      const tokensToDrop = prune;
      const prefixTokenCount = countTokens(activePrefix, helper.modelName);
      const suffixTokenCount = countTokens(activeSuffix, helper.modelName);
      ...
      if (totalContextTokens > 0) {
        ...
        activePrefix = pruneLinesFromTop(
          activePrefix,
          allowedPrefixTokens,
          helper.modelName,
        );
        activeSuffix = pruneLinesFromBottom(
          activeSuffix,
          allowedSuffixTokens,
          helper.modelName,
        );
        compiledPrefix = activePrefix;
        compiledSuffix = activeSuffix;
        prompt = llm.supportsFim()
          ? compiledPrefix
          : prompt;
      }
    } else if (llm.supportsFim()) {
      prompt = compiledPrefix;
    }
  }
