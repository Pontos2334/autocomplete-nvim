...
function modelLooksFimCapable(model: string): boolean {
  ...
  return FIM_MODEL_HINTS.some((hint) => lower.includes(hint));
}
...
class OpenAI extends BaseLLM {
  ...
  public maxStopWords: number | undefined;
  private readonly configuredProviderName: string | undefined;

  constructor(options: LLMOptions) {
    super(options);
    this.useLegacyCompletionsEndpoint = options.useLegacyCompletionsEndpoint;
    this.apiType = (options as any).apiType;
    this.apiVersion = (options as any).apiVersion ?? "2023-07-01-preview";
    this.deployment = (options as any).deployment;
    this.maxStopWords = (options as any).maxStopWords;
    this.configuredProviderName = (options as any).provider;
  }
  ...
  get providerName(): string {
    return this.configuredProviderName ?? OpenAI.providerName;
  }
  ...
  async *streamChat(
    ...
    const opts = { model: this.model, ...options };
    yield* this._streamChat(messages, signal, opts);
  }

  supportsFim(): boolean {
    if (!this.model || !this.apiBase) {
      return false;
    }

    if (!modelLooksFimCapable(this.model)) {
      return false;
    }
    ...
    if (host.includes("api.openai.com") || host.includes("azure")) {
      return false;
    }

    if (host === "api.deepseek.com") {
      return url.pathname.replace(/\/+$/, "") === "/beta";
    }
  ...
  async *streamFim(
    ...
    const opts = { model: this.model, ...options };
    yield* this._streamFim(prefix, suffix, signal, opts);
  }
  ...
  protected async *_streamFim(
    ...
    // DeepSeek uses standard /completions endpoint with beta base_url
    // Others use /fim/completions
    // Ensure apiBase ends with / so URL() appends instead of replacing last segment
    const apiBase = this.apiBase!.endsWith("/") ? this.apiBase! : this.apiBase! + "/";
    const endpoint = isDeepSeek
      ? new URL("completions", apiBase)
      : new URL("fim/completions", apiBase);

    log(`[FIM] POST ${endpoint.toString()} (model=${options.model}, deepSeek=${isDeepSeek})`);
    log(`[FIM] payload sizes: prompt=${prefix.length}chars, suffix=${suffix.length}chars`);
    log(`[FIM] stop=${JSON.stringify(options.stop)?.substring(0, 200)}, max_tokens=${options.maxTokens}, temp=${options.temperature}`);
    ...
    log(`[FIM] Response: ${resp.status} ${resp.statusText}, body=${!!resp.body}`);

    if (!resp.ok) {
      const errText = await resp.text();
      log(`[FIM] Error body: ${errText.substring(0, 300)}`);
      throw new Error(`FIM request failed: ${resp.status} - ${errText.substring(0, 200)}`);
    }
    ...
    for await (const chunk of streamSse(resp)) {
      if (firstChunk) {
        log(`[FIM] First SSE chunk: ${JSON.stringify(chunk).substring(0, 200)}`);
        firstChunk = false;
      }
      if (isDeepSeek) {
    ...
    log(`[FIM] Stream done, ${chunkCount} chunks`);
  }
